...
class TerminateUserTasksTest extends BaseTest {

    @Test
    @Deployment(resources = "benchmark-processes/terminateUserTasks.bpmn20.xml")
    void testProcess() {
        assertThat(historyService.createHistoricProcessInstanceQuery().count()).isZero();

        TerminateUserTasksRunnable runnable = new TerminateUserTasksRunnable(runtimeService, taskService);
        ...
        assertThat(historyService.createHistoricTaskInstanceQuery().finished().count()).isEqualTo(3);
    }
