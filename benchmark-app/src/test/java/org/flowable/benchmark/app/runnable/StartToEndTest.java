...
class StartToEndTest extends BaseTest {

    @Test
    @Deployment(resources = "benchmark-processes/startToEnd.bpmn20.xml")
    void testProcess() {
        assertThat(historyService.createHistoricProcessInstanceQuery().count()).isZero();

        StartToEndRunnable runnable = new StartToEndRunnable(runtimeService);
