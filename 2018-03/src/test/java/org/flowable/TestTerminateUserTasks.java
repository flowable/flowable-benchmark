...
public class TestTerminateUserTasks extends AbstractTest {
    
    @Test
    public void testProcess() {
        flowableRule.getRepositoryService().createDeployment().addClasspathResource("terminateUserTasks.bpmn20.xml").deploy();
        assertEquals(0, flowableRule.getHistoryService().createHistoricProcessInstanceQuery().count());
        
        TerminateUserTasksRunnable runnable = new TerminateUserTasksRunnable();
        runnable.run();
